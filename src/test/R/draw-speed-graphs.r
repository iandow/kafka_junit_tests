...
legend(x=32,y=2.1,legend=c(1,2,5,10,15,20), col=rainbow(6), fill=rainbow(6), title="Threads")
...
title("Message Size vs Throughput (Messages)")
dev.off()

png(file="size-count-bytes.png", width=800, height=500, pointsize=16)
x = read.csv("size-count.csv")
boxplot(batchRate*messageSize/1e6 ~ messageSize, x,
        ylab="Throughput (MB/Second)", xlab="Message Size (Bytes)", ylim=c(0,200),
        col=rainbow(6)[ceiling((1:36)/6)], xaxt='n')
axis(1,labels=as.character(rep(c(100,500,1000,2000,10000,50000,100000),7)), at=(1:49), las=3)
title("Message Size vs Throughput (MBs)")
dev.off()
